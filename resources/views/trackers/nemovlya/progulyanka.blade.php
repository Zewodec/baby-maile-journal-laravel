...
@section('title', 'Трекери прогулянки немовля')
...
@section('head')
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#progulyanka-form').submit(function (event) {
                ...
                var formData = {
                    _token: '{{ csrf_token() }}',
                    progulyanka_time: totalTime / 1000,
                    progulyanka_date: $('#date_time_input').val()
                }

                // Send AJAX request
                $.ajax({
                    url: '{{route('trackers.nemovlya.progulyanka.save')}}', // URL to submit the form data
                    type: 'POST',
                    data: formData,
...
<h1 class="h1_anastasia text-center">Прогулянка</h1>
...
<section class="goduvanya-grudy-section padding-bottom-140">
    ...
    <form id="progulyanka-form" action="{{route("trackers.nemovlya.son.save")}}" method="post">
